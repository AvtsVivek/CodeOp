...
async function decreasePrice(sku, price) {
    const url = `/sales/products/${sku}/decreasePrice`;
    await fetch(url, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
...
async function setAsAvailable() {
    ...
    const result = await fetch(item.dataset.href,{ method: 'POST' });
    if (result.status === 400) {
        const resultBody = await result.json();
        alert(resultBody.detail);
    } else {
        await loadSales();
    }
